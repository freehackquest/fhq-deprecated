...
class fhq_profile
{
	...
	function get_user_profile($user_id = 0)
	{
		...
		else
		{
			...
			$query = 'SELECT userquest.idquest, quest.subject, quest.name, quest.score FROM userquest LEFT JOIN quest ON quest.idquest=userquest.idquest WHERE iduser="'.$user_id.'"';
			$result = $db->query($query);
